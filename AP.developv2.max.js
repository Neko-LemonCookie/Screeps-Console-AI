...
const DevelopV2_Max = {
    ...
    run: function(room) { try { const s=this.analyze(room); if(!this._shouldRefresh(room)) return; this._publishCreepNeeds(room,s); } catch(e){console.log("[DevelopV2-Max] ERROR:",e);} },
    ...
    _publishCreepNeeds:function(room,state){const tb=require('lib.AP.taskboard');for(const m in this.CONFIG.CREEP_CONFIG){const c=this.CONFIG.CREEP_CONFIG[m];const cur=(state.creeps[m]||0);if(cur<c.minCount)tb.strategy.needCreeps(room.name,{model:m,count:c.minCount-cur,priority:m==='AttackerI'?'attack':m==='CarrierI'?'carry':'harvest',data:{bodySize:c.bodySize,enableBoost:c.enableBoost}});}},
