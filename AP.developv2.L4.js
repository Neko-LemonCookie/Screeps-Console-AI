...
const DevelopV2_L4 = {
    ...
    run: function(room) { try { if (Game.time % 2 !== 0) return; const s = this.analyze(room); this._publishCreepNeeds(room, s); } catch (e) { console.log("[DevelopV2-L4] ERROR:", e); } },
    ...
    _publishCreepNeeds: function(room, state) { const tb=require('lib.AP.taskboard'),cfg=this.CONFIG.CREEP_CONFIG.CommonI; if(state.commonICount>=cfg.maxCount)return; if(state.commonICount<cfg.minCount) tb.strategy.needCreeps(room.name,{model:'CommonI',count:cfg.minCount-state.commonICount,priority:'harvest',data:{bodySize:cfg.bodySize}}); },
